...
Die Schnittstelle einer Ressource muss nicht auf CRUD Funktionen reduziert sein, oder sie exakt in dieser Form umsetzen, dennoch wichtig ist dieses Konzept allema, da es häufig als Ursprung dient.
\\\\
...
\importantvs{
Die heute wichtigsten Architekturstile sind (individuelle Auswahl):
\begin{itemize}
    ...
    \item \textbf{gRPC}: Protokollbasiertes RPC mit hoher Effizienz.
...
\end{table}
\importantvs{
\paragraph{Ereignisgesteuerte Architektur (Event-Driven Architecture, EDA)}
\mbox{}\\
Statt eines klassischen Request-Response-Modells setzt EDA auf asynchrone Ereignisse. Komponenten sind über eine Messaging-Plattform (z. B. Kafka, RabbitMQ) verbunden. Dies ermöglicht lose Kopplung und verbesserte Skalierbarkeit. 
\\
\textbf{Vorteile:}
\begin{itemize}
    \item Hohe Skalierbarkeit und Asynchronität.
    \item Entkopplung der Kommunikation zwischen Diensten.
    \item Geeignet für reaktive Systeme mit Echtzeitverarbeitung.
\end{itemize}
}
\importantvs{
\paragraph{Service Mesh}
\mbox{}\\
Ein Service Mesh ersetzt herkömmliche API-Management-Schichten durch eine dedizierte Netzwerkschicht. Jedes Microservice verfügt über ein Sidecar-Proxy, das die gesamte Kommunikation übernimmt (z. B. Istio, Linkerd). Ein Sidecar Proxy ist ein Architekturkonzept, das häufig in Service Meshes verwendet wird. Es handelt sich dabei um eine zusätzliche Proxy-Instanz, die neben einem Hauptdienst (Microservice) in derselben Umgebung läuft – meist als separater Container in einem Kubernetes Pod. Der Sidecar Proxy übernimmt verschiedene Funktionen, insbesondere im Bereich Netzwerkkontrolle, Sicherheit und Observability.
Hierfür ist das Verständnis von Proxies unabdinglich. 
\\
\textbf{Vorteile:}
\begin{itemize}
    \item Automatische Service-to-Service-Kommunikation ohne API-Design.
    \item Verbesserte Resilienz, Sicherheit und Observability.
\end{itemize}
}
\importantvs{
\paragraph{Datenbank-Replikation und Direct Data Access}
\mbox{}\\
Statt API-Aufrufen können Systeme direkt auf eine geteilte Datenbank zugreifen. Daten werden synchron oder asynchron repliziert (z. B. PostgreSQL Logical Replication).
\\
\textbf{Vorteile:}
\begin{itemize}
    \item Reduktion von API-Aufrufen und Netzwerk-Overhead.
    \item Keine API-Wartung oder Versionierung erforderlich.
    \item Optimierte Performance für datenintensive Anwendungen.
\end{itemize}
}
\importantvs{
\paragraph{Peer-to-Peer (P2P)-Kommunikation}
\mbox{}\\
Im Gegensatz zu Client-Server-APIs setzt P2P-Kommunikation auf eine dezentrale Architektur, in der Knoten direkt miteinander interagieren (z. B. IPFS, libp2p).
\\
\textbf{Vorteile:}
\begin{itemize}
    \item Eliminierung zentraler Vermittler und Single Points of Failure.
    \item Höhere Resilienz durch verteilte Kommunikationsstrukturen.
    \item Besonders geeignet für Blockchain- und Filesharing-Anwendungen.
\end{itemize}
}
\importantvs{
\paragraph{Function-as-a-Service (FaaS)}
\mbox{}\\
FaaS ermöglicht die Ausführung einzelner Funktionen ohne persistente API-Schnittstellen. Die Verarbeitung wird durch Events getriggert (z. B. AWS Lambda, Google Cloud Functions).
\\
\textbf{Vorteile:}
\begin{itemize}
    \item Keine API-Verwaltung notwendig.
    \item Skalierung auf Abruf ohne dauerhafte Server-Ressourcen.
    \item Geeignet für serverlose, eventbasierte Systeme.
\end{itemize}
}
...
\importantvs{
Eine \textbf{direkte} Kopplung tritt auf, wenn Anwendungen oder Systeme direkt miteinander verbunden sind und direkt miteinander kommunizieren. Beispielsweise können zwei Anwendungen über eine Netzwerkverbindung oder einen direkten Datenlink verbunden sein und direkt miteinander kommunizieren.
}
\examplevs{
...
\importantvs{
Eine \textbf{losgekoppelte} Kopplung tritt auf, wenn Anwendungen oder Systeme über Nachrichten kommunizieren, ohne sich direkt zu verbinden. In diesem Fall sendet eine Anwendung eine Nachricht an eine Middleware oder einen Broker, der die Nachricht an die Empfängeranwendung weiterleitet.
}
\examplevs{
...
\importantvs{
Eine \textbf{strukturelle} Kopplung tritt auf, wenn Anwendungen oder Systeme durch gemeinsame Datenstrukturen oder Datentypen verbunden sind. Beispielsweise können Anwendungen über eine gemeinsame Datenbank oder eine gemeinsame Dateistruktur kommunizieren.
}
\examplevs{
\textbf{Analogie:} 
...
\examplevs{
\paragraph{AI Beispiel - Webserver Cluster}\mbox{}\\ 
...
In verteilten Systemen können Nachrichtenabhängigkeiten einen Einfluss auf die stateful- oder stateless-Eigenschaften eines Systems haben. 
\examplevs{Nachrichtenabhängigkeiten treten auf, wenn die Verarbeitung einer Nachricht von der Verarbeitung einer oder mehrerer vorheriger Nachrichten abhängig ist. Ein Beispiel sind dafür Parameter in Nachrichten die sich mit relativen Werten aufeinander beziehen. In solchen Fällen ist es erforderlich, den Zustand der Kommunikation zu verfolgen, um sicherzustellen, dass die Nachrichten in der richtigen Reihenfolge verarbeitet werden und das System korrekt funktioniert, dies führt implizit zu zustandsbehafteten Systemen.}
\begin{itemize}  
...
Auch in Verteilten Systemen bleibt vom Grundsatz die Idee, das Transaktionen die ACID-Eigenschaften (Atomicity, Consistency, Isolation, Durability) erfüllen. 
\importantvs{
\begin{itemize}
...
\end{itemize}
Ein Transaktionsmanager spielt eine entscheidende Rolle bei der Sicherstellung von Konsistenz, Integrität und Fehlertoleranz in verteilten Systemen, wenn Transaktionen verwendet werden. So ist es z.B. denkbar, eine Urlaubsreise nicht als eigenständigen Dienst zu implementieren, sondern Hotelzimmer, Mietwagen und Flug zu einem neuen Dienst zusammenzufassen, der vorher nicht existierte einzig mit einem Transaktionskonzept.
\examplevs{
\textbf{Beispiel im Detail}\\\\
...
\importantvs{
Die Eigenschaften von Message Passing können wie folgt zusammengefasst werden:
\begin{itemize}
\item \textbf{Asynchrone Kommunikation:} Die Kommunikation ist in der Regel asynchron, wodurch Prozesse unabhängig voneinander weiterarbeiten können, ohne auf Antworten warten zu müssen.
\item \textbf{Lose Kopplung:} Die Kommunikation zwischen Prozessen ist indirekt und erfordert keine direkte Kenntnis der beteiligten Prozesse, was zu einer losen Kopplung der Komponenten führt.
\item \textbf{Skalierbarkeit:} Message Passing eignet sich gut für verteilte Systeme mit einer großen Anzahl von Prozessen oder Komponenten, da es einfacher ist, zusätzliche Knoten hinzuzufügen und die Kommunikation zwischen ihnen zu verwalten.
\end{itemize}
}
\examplevs{
Beispiele für Message Passing in verteilten Systemen:
\begin{itemize}
\item \textbf{Message Queues:} Message Queues sind Datenstrukturen, die zum Speichern und Übertragen von Nachrichten zwischen verteilten Prozessen verwendet werden. Beispiele für Message Queue-Systeme sind RabbitMQ, Apache Kafka und Amazon SQS. Sie ermöglichen die asynchrone Kommunikation und die Entkopplung von sendenden und empfangenden Prozessen.
\end{itemize}
}

\examplevs{
\begin{itemize}
\item \textbf{Message Passing Interface (MPI):} MPI ist ein Standard für die Kommunikation zwischen Prozessen in verteilten Systemen, insbesondere in High-Performance-Computing-Umgebungen. MPI stellt Funktionen zum Senden und Empfangen von Nachrichten, Synchronisation von Prozessen und Gruppenkommunikation bereit.

\item \textbf{Publish-Subscribe-Systeme:} In diesem Modell senden Publisher Nachrichten an Topics, während Subscriber Nachrichten von den Topics empfangen, die sie abonniert haben. Beispiele für Publish-Subscribe-Systeme sind Google Cloud Pub/Sub, Apache Pulsar und MQTT. Dieses Modell ist ereignisgetrieben und ermöglicht eine lose Kopplung der beteiligten Prozesse.
\end{itemize}
}
...
\importantvs{
Peer-to-Peer (P2P) bezeichnet in verteilten Systemen eine Netzwerkarchitektur, bei der alle Teilnehmer (Peers) gleichberechtigt miteinander interagieren können. Im Gegensatz zum Client-Server-Modell, bei dem ein zentraler Server Daten und Dienste bereitstellt, agieren Peers in einem P2P-Netzwerk sowohl als Server als auch als Client.
}
...
\examplevs{
Wenn verschiedene Optionen zur Verfügung stehen, aber noch keine Erfahrungen mit der Umsetzung vorliegen, sollte mit einem Schichtenmodell begonnen werden.
}
...
\examplevs{
\textbf{Function as a Service (FaaS)}  ist ein Paradebeispiel für cloud-native Anwendungen, die speziell für den Betrieb in Cloud-Umgebungen optimiert sind und Skalierbarkeit, Resilienz sowie lose Kopplung fördern.
}
\end{itemize}  
...
\importantvs{
Unter einer node versteht man ein autonomes Computerelement, sei es Hardware-Systeme oder Software-Prozesse. 
}
...
\examplevs{
   Bei der Kapselung treten immer wieder Verständnisschwierigkeiten auf. Ein Beispiel, das mit der Erfahrung eines Moduls wie Computernetzwerke verstanden werden kann, ist die Trennung der Verantwortlichkeiten der Anwendungsschicht und der Transportschicht in einem Netzwerkstack. TCP (Transmission Control Protocol) und HTML (HyperText Markup Language) arbeiten in verschiedenen Schichten des OSI-Schichtenmodells, was die Kapselung von Daten ermöglicht. TCP gehört zur Transportschicht und sorgt für eine zuverlässige Datenübertragung. HTML arbeitet in der Anwendungsschicht und definiert die Struktur der Web-Inhalte. Die Kapselung erfolgt, indem die HTML-Daten von TCP in Segmente aufgeteilt und transportiert werden, wobei jede Schicht nur ihre spezifischen Funktionen erfüllt.
}
\mbox{}\\ 
...
\begin{figure}[b!] % Das H platziert die Grafik exakt an der Stelle
    \centering
...
\\\\
Um nochmals die Kraft eines Schichtenmodells zu verdeutlichen, soll auch nochmal diskutiert werden, nach welchen Ansätzen ein System in Schichten unterschieden werden kann. Doch hierzu soll weniger der deutsche Begriff Schicht genutzt sein, sondern vielmehr  die englischen Begriffe \enquote{Tier} (Ebene) und \enquote{Layer} (Schicht). Diese Begriffe sind in dem Kontext gut etabliert und besser geeignet, um die notwendige Diskussion zu führen:
\newpage
\paragraph{Layer}\mbox{}\\
\importantvs{
Ein (Software-)\textbf{Layer} beschreibt eine logische Gruppierung von Funktionen innerhalb einer Anwendung oder eines Systems. 
}
\paragraph{Layer}\mbox{}\\
...
\paragraph{Tier}\mbox{}\\
\importantvs{
Ein \textbf{Tier} beschreibt die physische oder logische Aufteilung von Anwendungen oder Systemen auf verschiedene Server oder Endgeräte.
}
\mbox{}\\
...
\importantvs{Das Single-Node-Pattern bezieht sich auf die Verwendung eines einzigen Serverknotens oder einer einzelnen Instanz des zu erbringenden Dienstes, anstatt mehrere Knoten zu verwenden, um denselben Dienst bereitzustellen.} 

\examplevs{
Hier sind einige Beispiele von Unternehmen, die dieses Muster einsetzen:
\begin{itemize}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \def\layerwidth{0.75\textwidth}
        \def\layerheight{2cm}

        % Application Layer
        \draw[fill=blue!30] (0, 0) rectangle (\layerwidth, \layerheight) node[midway] {Applikation};

        % Middleware Layer
        \draw[fill=green!30] (0, -\layerheight) rectangle (\layerwidth, 0) node[midway] {Middleware};
...
\end{itemize}
\importantvs{Ein großer Vorteil ist, das für den Applikationsentwickler nur die Entwicklung des Anwendungsprotokolls bleibt. Im besten Fall bietet die Schnittstelle der Middlewareschicht ein abstrahiertes \enquote{Interface}, welches sich im besten Fall nicht von einem Standalone System abgrenzen lässt, sich somit verhält wie ein kohärentes System.}
...
\\\\
Ein anderes Beispiel wäre ein System zur Überwachung von Sensoren in einer Fabrik. Der Absender kann eine Point-to-Multipoint-Nachricht an alle Sensoren senden, um ein Update der Einstellungen oder einen Alarm zu senden, anstatt eine Point-to-Point-Nachricht an jeden Sensor einzeln zu senden.  
}
...
\importantvs{
    Zustandslos in VS einfach gesprochen: Keine Abhängigkeit der Nachrichten zueinander!
}
...
\begin{itemize}
\item \textbf{Event-Producer}: Komponenten, die Ereignisse generieren und in das System einspeisen.
\item \textbf{Event-Channel}: Eine Kommunikationsinfrastruktur, die Events von Producern zu Consumern transportiert, z. B. Message Broker oder Event Bus.
\item \textbf{Event-Consumer}: Komponenten, die auf bestimmte Ereignisse reagieren und entsprechende Aktionen ausführen.
\end{itemize}
...
\examplevs{
Auch Architekturmuster sind wesentlicher Teil der Informatikausbildung, als Beispiele können genannt werden:
\begin{itemize}
...
\examplevs{
\textbf{Hinweis:} Es gibt sehr gute Bücher wie \textbf{Patterns of Distributed Systems} von Martin Fowler aus dem Pearson Addison-Wesley Verlag ISBN-13: 978-0138221980. Der Umfang dieser Ausarbeitungen würden die eines Lehrmoduls sprengen, das sich nur auf eine Einführung konzentriert. Für die praktische Heranführung gibt es begleitend zu den Pattern dieser Ausarbeitung ein github Repository \url{https://github.com/scimbe/VS_Pattern_By_KI}, welches die wesentlichen Sourcen in Java umfasst. 
}
...
\examplevs{
\paragraph{Das Fallbeispiel}\mbox{}\\    
Das Szenario für die Fallstudie soll eine einfache Led-Lampe sein, die über eine eigene CPU und eigenen Speicher und einen Bluetooth Sender und Empfänger verfügt. Der Aufbau ist erweitert durch einen Raspberry PI, der sowohl die Anbindung der Lampe über Bluetooth repräsentiert, als auch den Servicezugriffspunkt für Applikationen im Internet. Der Raspberry Pi kann über eine Global Unicast IPv6 Adresse erreicht werden, um Steuerungssignale für die Lampe über die App abzugeben. 
}
...
\warningvs{ 
\paragraph{Forwarding-Broker}\mbox{}\\
...
\warningvs{ 
\paragraph{Handle-Driven-Broker}\mbox{}\\
...
In dem Fallbeispiel der Lampe soll zur Verdeutlichung die einfache Sequenz des Hinzufügen eines Observers zwischen PI (Controller) und Lampe (Model) den Nutzen verdeutlichen. Im Sequenzdiagramm~\ref{fig:seq-mvc-observer} sind die Schritte zum Hinzufügen eines Observers wie folgt beschrieben:
...
\examplevs{
\paragraph{Praktische Umsetzung im Web}\mbox{}\\
...
\begin{figure}[htb]
  \centering
  \includegraphics[width=0.40\textwidth]{fig/uml/callback-class.png}
...
\end{itemize}
\examplevs{
 Ein Singleton-Pattern kann für verschiedene Anforderungen in einem verteilten System unterschiedliche Auswirkungen haben. Einige sollen exemplarisch diskutiert werden.
\begin{itemize}
...
Im Kontext dieser Diskussion kann festgestellt werden, das die Eignung des Singleton-Patterns in verteilten Systemen von den spezifischen Anforderungen und der Architektur des Systems abhängt, seltener existiert, aber immer wieder auftritt. 
...
%\end{figure}
\examplevs{
Das Factory-Pattern in VS bietet eine Vielzahl von Herausforderungen:
\begin{itemize}
...
Dependency Injection (DI) ist ein nützliches Muster für die Entwicklung von Anwendungen. 
\examplevs{
...
\examplevs{
Diese zunächst einfache und naive Darstellung ist als Möglichkeit gedacht, um einfach einen Überblick zu schaffen. Natürlich müssen hier verschiedene Aspekte verfeinert und für das bessere Verständnis nochmal diskutiert werden. Hier wird auf das Praktikum verwiesen.
}
\importantvs{
\paragraph{Modellierung je Schicht, nicht über die Schichten}\mbox{}\\
Man muss aber festhalten, dass aus der naiven Ansicht der statischen Komponentensicht häufig ein Verhalten abgeleitet wird, welches auf einer missverstandenen Interpretation der Architektur zurückzuführen ist, weniger auf ein noch nicht eingeführtes Verhalten. Ist die Beschreibung des Verhaltens in der Architektur das Ziel, sollte nochmals auf die Nutzung des Schichtenmodells hingewiesen werden. Jede Schicht kapselt die Sequenzen nach der zugeordneten Verantwortlichkeit.
...
\paragraph{Copy in, Copy out}\mbox{}\\
Das Ziel, den Marshalling-Prozess möglich generisch zu gestallten, ist in dem Prinzip \enquote{Copy in, Copy out} gut repräsentiert. Der Begriff \enquote{Copy in, Copy out} (auch als \enquote{Call by Value-Result} bezeichnet) ist eine Semantik zur Übergabe von Parametern bei entfernten Prozeduraufrufen (RPCs) oder in Programmiersprachen. Es beschreibt die Art und Weise, wie Daten und Parameter zwischen dem aufrufenden und dem aufgerufenen System übertragen werden. Hier ist das grundlegende Prinzip von \enquote{Copy in, Copy out}:
\begin{itemize} 
...
\paragraph{ Skeleton, Server-Stub und Server-Proxy }\mbox{}\\
